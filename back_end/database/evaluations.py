...
class EvalsScraper():
    ...
    def get_evals(self, dept, course_id, term):
        ...
        if not self._is_correct_term(response.url, term):
            raise ValueError(f"No evaluations yet for course {course_id} in term {term}. URL was redirected to evaluations page for a different term.")
        ...
        if len(course) != 1:
            raise AssertionError(f"Expected to only find one div with classname=course.You should probably check the code and make sure it's up to date.")
        course = course[0]
